            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`text-sm font-medium transition-colors hover:text-gold ${
                  isActive(item.href) ? 'text-gold' : 'text-gray-700'
                }`}
              >
                {item.name}
              </Link>
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="w-80">
                <div className="flex flex-col space-y-4 mt-8">
                  <Link to="/" className="text-2xl font-bold arabic-heading text-center pb-4 border-b">
                    <span className="text-luxury-black">لكس</span>
                    <span className="text-gold mr-1">آريا</span>
                  </Link>
                  ...
                  <Link
                    to="/admin"
                    onClick={() => setIsOpen(false)}
                    className="text-lg font-medium py-2 px-4 rounded-lg text-gray-700 hover:bg-gray-50 border-t pt-4 mt-4"
                  >
